...
fixed_hands = {
    "picam++": ("El puño cerrado.", ["c", "c", "c", "c", "c"]),
    "pir-O": (
        "El índice recto, con el pulgar pegado. El resto de dedos estirados",
        ["f", "f", "f", "f", "f"],
    ),
    "TE": (
        "El índice y el pulgar cruzados, el resto de dedos estirados.",
        ["t", "t", "t", "t", "t"],
    ),
    "CI": ("Los dedos índice y corazón cruzados.", ["x", "x", "x", "x", "x"]),
    "pi-O": (
        "La punta del índice tocando la falange del pulgar, el resto de dedos extendidos.",
        ["s", "s", "s", "s", "s"],
    ),
}
...
def hand_shape(sh):
    ...
    finger_params = ["E" if others else "c" for _ in ["P", "I", "C", "A", "M"]]
